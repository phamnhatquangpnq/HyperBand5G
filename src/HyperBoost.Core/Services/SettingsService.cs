...
public class SettingsService : ISettingsService
{
    ...
    private const string AppName = "HyperBoost";
    ...
    public SettingsService(string? customPath = null)
    {
        ...
        else
        {
            ...
            string dir = Path.Combine(appData, "HyperBoost");
    ...
    public async Task<AppSettings> LoadSettingsAsync()
    {
        try
        {
            if (File.Exists(_settingsFilePath))
            {
                ...
                if (settings != null)
                {
                    if (settings.Min5GhzQualityThreshold == 35)
                    {
                        settings = settings with { Min5GhzQualityThreshold = 15 };
                    }
                    return settings;
                }
            }
