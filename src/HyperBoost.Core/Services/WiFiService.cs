...
// Dependencies: System.Diagnostics, HyperBoost.Core.Models
...
public class WiFiService : IWiFiService
{
    ...
    public virtual async Task<bool> SetAdapterPreferredBand5GHzAsync()
    {
        try
        {
            // PowerShell script to set Wi-Fi adapter advanced property 'Preferred Band' to 5 GHz without resetting card
    ...
    public virtual async Task<bool> ResetAdapterPreferredBandAsync()
    {
        try
        {
            // PowerShell script to reset Wi-Fi adapter advanced property 'Preferred Band' to Auto / 0 (No Preference) without resetting card
            string psCommand = "Get-NetAdapter | Where-Object { $_.InterfaceDescription -like '*Wi-Fi*' -or $_.Name -like '*Wi-Fi*' } | ForEach-Object { Set-NetAdapterAdvancedProperty -Name $_.Name -RegistryKeyword '*PreferredBand' -RegistryValue '0' -ErrorAction SilentlyContinue }";
            await RunCommandAsync("powershell", $"-NoProfile -ExecutionPolicy Bypass -Command \"{psCommand}\"");
            return true;
        }
        catch (Exception ex)
        {
            Console.WriteLine($"Error resetting adapter preferred band: {ex.Message}");
            return false;
        }
    }
