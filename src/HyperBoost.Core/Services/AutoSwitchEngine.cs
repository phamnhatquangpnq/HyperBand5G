...
// Dependencies: System, HyperBoost.Core.Models, HyperBoost.Core.Services
...
public class AutoSwitchEngine
{
    ...
    public virtual async Task<SwitchEventLog?> EvaluateAndSwitchAsync()
    {
        ...
        if (current.Band == WiFiBand.Band24GHz)
        {
            ...
            int effectiveThreshold = Config.Min5GhzQualityThreshold;
            var best5Ghz = bssids
                .Where(b => string.Equals(b.Ssid, current.Ssid, StringComparison.OrdinalIgnoreCase) &&
                            ...
                            b.SignalQuality >= effectiveThreshold)
                .OrderByDescending(b => b.Score)
                .FirstOrDefault();

            if (best5Ghz != null)
            {
                if (Config.Prefer5GhzAdapterProperty)
                {
